...
- ✅ Sizzle video: `hyperframes check` and a full `hyperframes render` (2,477 frames) plus the final mux — that is how the video at the top of this page was produced.
- ⚠️ The ElevenLabs steps (`gen_voice`, `gen_sfx`, full `mix`) for both video skills were not run, to avoid spending credits. The sizzle scripts were lightly ported (paths, `ffmpeg` lookup, missing folders) and are not re-verified end to end.
